...
class vEdge_vm(vrnetlab.VM):
    def __init__(self, username, password, **kwargs):
        ...
        super(vEdge_vm, self).__init__(username, password, disk_image=disk_image, ram=2048)
        ...
        self.vbond_local = kwargs.get('vbond_local')
        self.num_nics = 2
    ...
    def bootstrap_spin(self):
        ...
        if match: # got a match!
            if ridx == 0: # System Ready
                ...
                self.wait_write("admin", wait="vedge login:")
    ...
    def initial_config(self):
        """ Deal with a couple of setup questions after which
            the vEdge reboots and is ready for use
        """
    ...
    def bootstrap_config(self):
        ...
        if self.vbond_local:
            self.wait_write("host-name vbond")
            self.wait_write("vbond %s local" % (self.vbond))
        else:
        ...
        self.wait_write("top")
        self.wait_write("vpn 0 interface ge0/0")
...
class vEdge(vrnetlab.VR):
    def __init__(self, username, password, **kwargs):
        super(vEdge, self).__init__(username, password)
        self.vms = [ vEdge_vm(username, password, **kwargs) ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--system-ip', default=os.getenv('SYSTEM_IP', '10.0.1.100'), help='System IP address')
    parser.add_argument('--transport-ipv4-address', default=os.getenv('TRANSPORT_IPV4_ADDRESS', '10.0.1.1/24'), help='IPv4 address and prefix length on first transport interface')
    parser.add_argument('--transport-ipv4-gateway', default=os.getenv('TRANSPORT_IPV4_GATEWAY', '10.0.1.254'), help='IPv4 default gateway')
    parser.add_argument('--site-id', default=os.getenv('SITE_ID', 100), help='SD-WAN Site ID')
    parser.add_argument('--vbond-local', default=vrnetlab.bool_from_env('VBOND_LOCAL', True), action='store_true', help='Whether this vEdge is a vBond')
    ...
    vr = vEdge(args.username, args.password,
