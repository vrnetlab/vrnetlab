...
if __name__ == '__main__':
    ...
    parser.add_argument('--refresh', help='refresh interval for roa cache', default='15')
    ...
    # start gortr
    cmd = ["/gortr", "--cache", "/roas.json", "-verify=false", "-checktime=false", "-refresh", args.refresh]
    gortrp = subprocess.Popen(cmd)
    while True:
        if gortrp.poll() == 0:
            print("gortr stopped, restarting in 2s")
            time.sleep(2)
            gortrp = subprocess.Popen(cmd)
        time.sleep(1)
